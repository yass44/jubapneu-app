...
# On essaie d'importer les secrets locaux, sinon on ignore (pour le Cloud)
try:
...
# --- CONNEXION SUPABASE (COMPATIBLE CLOUD & LOCAL) ---
@st.cache_resource
def init_connection():
    try:
        # 1. On essaie de lire depuis le coffre-fort Streamlit Cloud
        if "SUPABASE_URL" in st.secrets:
            url = st.secrets["SUPABASE_URL"]
            key = st.secrets["SUPABASE_KEY"]
        # 2. Sinon, on lit depuis le fichier local secrets_config.py
        elif secrets_config:
            url = secrets_config.SUPABASE_URL
            key = secrets_config.SUPABASE_KEY
        else:
            st.error("Aucune clé Supabase trouvée !")
            return None
...
def load_all_data():
    ...
    # On charge tout (sans cache pour modifications temps réel)
...
# --- MOTEUR ANALYSE PDF (FOURNISSEUR DELDO) ---
def analyser_ligne_deldo(description):
    infos = {
        "valid": False, "dimension_complete": description,
        "largeur": None, "hauteur": None, "diametre": None,
        "charge": "", "vitesse": "", "saison": "Été", "marque": "Inconnue"
    }
    # REGEX "SNIPER"
    ...
    if match:
        infos["valid"] = True
        infos["largeur"] = int(match.group(1)); infos["hauteur"] = int(match.group(2)); infos["diametre"] = int(match.group(3))
...
if page == "Stock Actuel":
    ...
    if not df_stock.empty:
        ...
        st.dataframe(df_view[['dimension_complete', 'stock_actuel', 'marque', 'saison', 'charge', 'vitesse', 'pmp_achat']], 
                     column_config={"stock_actuel": st.column_config.NumberColumn("En Stock", format="%d 🛞"), 
                                    "pmp_achat": st.column_config.NumberColumn("PMP Achat", format="%.2f €")},
    ...
    if uploaded_file is not None:
        ...
        with pdfplumber.open(uploaded_file) as pdf:
            for page_pdf in pdf.pages:
                ...
                for ligne in text.split('\n'):
                    ...
                    if match_ligne:
                        qte = int(match_ligne.group(1))
                        desc_brute = match_ligne.group(2)
                        prix = float(match_ligne.group(3))
                        
                        infos = analyser_ligne_deldo(desc_brute)
                        if infos["valid"]:
                            articles_trouves.append({
                                "Description": infos['dimension_complete'],
                                "Marque": infos['marque'],
                                "Qté": qte,
                                "Prix Achat": prix,
                                "Saison": infos['saison'],
        ...
        if articles_trouves:
            df_preview = pd.DataFrame(articles_trouves)
            st.success(f"✅ {len(articles_trouves)} lignes de pneus détectées !")
            st.dataframe(df_preview[['Description', 'Marque', 'Qté', 'Prix Achat']], use_container_width=True)
            ...
            if st.button("🚀 CONFIRMER L'IMPORTATION ET METTRE À JOUR LE STOCK", type="primary"):
                ...
                for i, item in enumerate(articles_trouves):
                    ...
                    if res.data:
                        ...
                        exist = res.data[0]
                        art_id = exist['id']
                        old_stock = exist['stock_actuel']
                        old_pmp = float(exist['pmp_achat'] or 0)
                        
                        new_stock = old_stock + qte
                        new_pmp = ((old_stock * old_pmp) + (qte * prix)) / new_stock if new_stock > 0 else prix
                        
                        supabase.table('articles').update({'stock_actuel': new_stock, 'pmp_achat': new_pmp}).eq('id', art_id).execute()
                    else:
                        # INSERT
                        new_art = {
                            'dimension_complete': infos['dimension_complete'],
                            'largeur': infos['largeur'], 'hauteur': infos['hauteur'], 'diametre': infos['diametre'],
                            'charge': infos['charge'], 'vitesse': infos['vitesse'], 'marque': infos['marque'],
                            'saison': infos['saison'], 'stock_actuel': qte, 'pmp_achat': prix
                        }
                        res_ins = supabase.table('articles').insert(new_art).execute()
                        art_id = res_ins.data[0]['id']
                    
                    # 2. Historique
                    supabase.table('mouvements_stock').insert({
                        'article_id': art_id, 'type_mouvement': 'ACHAT',
                        'quantite': qte, 'prix_achat_unitaire': prix,
                        'lien_facture_fournisseur': uploaded_file.name,
                        'created_at': datetime.now().strftime("%Y-%m-%d")
                    }).execute()
                    
                    progress_bar.progress((i + 1) / len(articles_trouves))
                
                st.success("🎉 Importation terminée ! Le stock est à jour.")
                st.balloons()
                # On vide le cache pour voir le stock à jour
                st.cache_resource.clear()
        else:
            st.warning("⚠️ Aucune ligne de pneu valide trouvée. Vérifiez que c'est bien une facture Deldo.")

elif page == "Historique Mouvements":
    st.title("📜 Historique Mouvements")
    if not df_hist.empty:
        df_hist['created_at'] = pd.to_datetime(df_hist['created_at'])
        st.dataframe(df_hist.sort_values('created_at', ascending=False), use_container_width=True)
...
# =========================================================
    ...
    if st.session_state.facture_reussie:
        ...
        st.balloons()
        st.success(f"✅ Facture N° {succes['num']} validée !")
        ...
        with c3: 
            if st.button("🔄 Nouvelle vente"):
                st.session_state.facture_reussie = None; st.session_state.panier = []; st.rerun()

    else:
        # CLIENT
        c_cl, c_info = st.columns([1, 2])
        with c_cl:
            opt_new = "➕ Nouveau Client"
            liste_cl = [opt_new] + (df_clients['nom'].tolist() if not df_clients.empty else [])
            choix_cl = st.selectbox("Client", liste_cl)
        
        info_client = {}
        with c_info:
            if choix_cl == opt_new:
                c1, c2 = st.columns(2)
                nom = c1.text_input("Nom *"); tel = c2.text_input("Tél")
                adr = st.text_input("Adresse"); cp = c1.text_input("CP"); ville = c2.text_input("Ville")
                info_client = {"nom": nom, "telephone": tel, "adresse": adr, "code_postal": cp, "ville": ville, "id": None}
            else:
                cl = df_clients[df_clients['nom'] == choix_cl].iloc[0]
                st.success(f"Client : {cl['nom']}")
                info_client = cl.to_dict()
        ...
        with col1:
            st.subheader("🛞 Pneus")
            if not df_stock.empty:
                l_pneus = ["-- Choix --"] + df_stock[df_stock['stock_actuel']>0]['dimension_complete'].unique().tolist()
                ch_pneu = st.selectbox("Choisir Pneu", l_pneus)
                if ch_pneu != "-- Choix --":
                    row = df_stock[df_stock['dimension_complete']==ch_pneu].iloc[0]
                    st.caption(f"Stock: {row['stock_actuel']} | PMP: {row['pmp_achat']:.2f}€")
                    qte = st.number_input("Qté", 1, int(row['stock_actuel']), 2, key="q_p")
                    px = st.number_input("Prix Vente (€)", value=float(f"{(row['pmp_achat'] or 0)*1.4:.2f}"), key="p_p")
                    if st.button("➕ Ajouter Pneu", type="secondary"):
                        st.session_state.panier.append({"type": "PNEU", "id": int(row['id']), "desc": f"Pneu {row['marque']} {row['dimension_complete']}", "qte": qte, "prix": px, "cout": float(row['pmp_achat'] or 0)})
                        st.success("Ajouté !"); st.rerun()

        with col2:
            st.subheader("🔧 Services")
            if not df_services.empty:
                l_serv = ["-- Choix --"] + df_services['description'].tolist()
                ch_serv = st.selectbox("Choisir Service", l_serv)
                if ch_serv != "-- Choix --":
                    s_row = df_services[df_services['description']==ch_serv].iloc[0]
                    qte_s = st.number_input("Qté", 1, 10, 2, key="q_s")
                    px_s = st.number_input("Prix (€)", value=float(s_row['prix_unitaire']), key="p_s")
                    if st.button("➕ Ajouter Service", type="secondary"):
                        st.session_state.panier.append({"type": "SERVICE", "id": None, "desc": s_row['description'], "qte": qte_s, "prix": px_s, "cout": 0})
                        st.success("Ajouté !"); st.rerun()
        ...
        if st.session_state.panier:
            ...
            df_p['Total'] = df_p['qte'] * df_p['prix']
            st.dataframe(df_p[['desc', 'qte', 'prix', 'Total']], use_container_width=True)
            col_tot, col_valid = st.columns([2, 1])
            total = df_p['Total'].sum()
            col_tot.metric("TOTAL A PAYER", f"{total:.2f} €")
            ...
            if col_valid.button("✅ VALIDER LA VENTE", type="primary", use_container_width=True):
                if not info_client['nom']: st.error("Client manquant !")
                    ...
                    num = f"FV-{datetime.now().strftime('%y%m-%H%M')}"
                    fid = supabase.table('factures_entete').insert({"client_id": cid, "total_ttc": total, "numero_facture": num, "statut": "Payée"}).execute().data[0]['id']
                    ...
                    pdf = generer_pdf(fid, info_client, st.session_state.panier, total, num)
                    st.session_state.facture_reussie = {"num": num, "pdf": pdf, "client": info_client['nom']}
                    st.rerun()
    ...
    if not df_factures.empty:
        df_disp = df_factures.copy()
        df_disp['Date'] = df_disp['created_at'].apply(lambda x: x[:10])
        df_disp['Client'] = df_disp['clients'].apply(lambda x: x['nom'] if x else 'Inconnu')
        st.dataframe(df_disp[['numero_facture', 'Date', 'Client', 'total_ttc', 'statut']], use_container_width=True)
        st.write("---")
        ...
        if st.button("Générer PDF"):
            ...
            pdf = generer_pdf(row['id'], row['clients'], lignes_fmt, row['total_ttc'], row['numero_facture'])
            st.download_button("Télécharger PDF", pdf, f"Facture_{sel_fact}.pdf", "application/pdf")
    ...
    if not df_clients.empty:
        ...
        if st.button("💾 Sauvegarder Clients"):
            ...
            st.success("Clients mis à jour !"); import time; time.sleep(1); st.rerun()
    ...
    with st.expander("➕ Créer"):
        with st.form("add_serv"):
            ...
            d=c1.text_input("Nom"); p=c2.number_input("Prix",0.0,100.0,15.0); c=c3.selectbox("Type",["Montage","Pièce"])
    ...
    if not df_services.empty:
        ...
        if st.button("💾 Sauvegarder Services"):
            ...
            st.success("Mis à jour !"); import time; time.sleep(1); st.rerun()
...
# =========================================================
    ...
    mode = st.radio("Vue", ["Jour", "Semaine", "Mois"], horizontal=True)
    rule = 'D' if mode=="Jour" else 'W' if mode=="Semaine" else 'M'
